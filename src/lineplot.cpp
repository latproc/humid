...
TimeSeries::TimeSeries()
    : mColor(255, 255), data(0), /*t_start(TIME_ANY), t_end(TIME_ANY),*/
      min_v(0.0), max_v(1.0), v_scale(1.0), h_scale(1.0),
      v_offset(0.0), /*h_offset(0.0), frozen(false)*/
...
float TimeSeries::getXScale() { return h_scale; }
...
LinePlot::LinePlot(Widget *parent, const std::string &caption)
    : Widget(parent), mCaption(caption), overlay_plots(false), start_time(microsecs()), x_min(0),
      x_scale(1.0), x_scroll(0.0), frozen(false), buffer_size(200), grid_intensity(0.05),
      display_grid(true) {
...
Vector2i LinePlot::preferredSize(NVGcontext *) const { return Vector2i(180, 45); }
...
TimeSeries *LinePlot::getTimeSeries(std::string name) {
    for (auto series_ptr : data)
        if (series_ptr->getName() == name)
            return series_ptr;
    return 0;
}

void LinePlot::addTimeSeries(TimeSeries *series) { data.push_back(series); }
...
void LinePlot::setMasterSeries(TimeSeries *series) { master_series = series; }
...
void TimeSeries::draw(NVGcontext *ctx, Vector2i &pos, Vector2i &size, const Axis &y_axis,
    ...
    if (show_name && getName().length()) {
        ...
        nvgTextAlign(ctx, NVG_ALIGN_LEFT | NVG_ALIGN_TOP);
        nvgFillColor(ctx, Color(255, 255, 255, 255));
        nvgText(ctx, pos.x() + 3, pos.y() + getYOffset() + (double)height / 2.0, getName().c_str(),
                NULL);
    }
    if (buf && buf->length() > 0) {
...
static std::string display_time(double dt) {
    ...
    else if (fabs(dt) > 1000.0)
        snprintf(buf, 20, "%7.3lfs", dt / 1000.0);
    else
        snprintf(buf, 20, "%5.0lfms", dt);
    return buf;
}

void LinePlot::drawGrid(NVGcontext *ctx, int x_start, double x_step, int num_x_steps, int y_start,
                        double y_step, int num_y_steps) {
...
void LinePlot::draw(NVGcontext *ctx) {
    ...
    if (display_grid && (overlay_plots || data.size() == 0))
        drawGrid(ctx, indent, displayed_time_range * time_scale / 10.0, 10, mPos.y() + top_indent,
                 (double)height / num_gridlines_v, num_gridlines_v);
            ...
            if (!overlay_plots) {
                ...
                if (display_grid && !overlay_plots)
                    drawGrid(ctx, indent, displayed_time_range * time_scale / 10.0, 10,
                             plot_pos + top_indent, (double)height / num_gridlines_v,
                             num_gridlines_v);
            }
...
bool LinePlot::mouseMotionEvent(const nanogui::Vector2i &p, const nanogui::Vector2i &rel,
    ...
    for (auto ts : data) {
        ...
        double val = ts->getData()->getBufferValueAt(t_data);
        char buf[80];
        snprintf(buf, 80, "%s%s: %5.2lf", sep, ts->getName().c_str(),
                 /* display_time(t_data).c_str(), */ val);
        sep = "\n";
        tooltip += buf;
    }
    setTooltip(tooltip);
...
void LinePlot::setMonitors(UserWindow *user_window, std::string items_to_monitor) {
    ...
    Color colors[] = {Color(200, 0, 0, 255), Color(0, 200, 0, 255), Color(0, 200, 200, 255),
        ...
        while (j < n) {
            ...
            if (pos[j] > -1) {
                ...
                if (t == earliest) {
                    pos[j]--;
                    if (pos[j] == -1)
                        --outstanding; // hit the end of this buffer
                }
            }
