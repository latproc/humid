...
Editor *Editor::instance() { return _instance; }
...
bool Editor::isEditMode() { return mEditMode; }

bool Editor::isCreateMode() { return mCreateMode; }

void Editor::setCreateMode(bool which) { mCreateMode = which; }
...
void Editor::save() {
    ...
    for (auto item : structure_files) {
		...
		*/
        ...
        if (sc)
            std::cout << "attempting to write structure class " << sc->getName() << " to " << fn
                      << "\n";
        ...
        if (s)
            std::cout << "attempting to write structure instance " << s->getName() << "\n";
