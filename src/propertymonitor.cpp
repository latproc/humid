    ...
    if (dh && dh->getTarget()) {
        ...
        case Handle::POSITION: {
            ...
            if (target_parent && target_parent->contains(newPos + target_parent->position()) &&
                target_parent->contains(newPos + dh->getTarget()->size() +
                                        target_parent->position())) {
            ...
            int y = newPos.y();
            if (y < target_parent->theme()->mWindowHeaderHeight + 1) {
                y = target_parent->theme()->mWindowHeaderHeight + 1;
            }
            if (y > target_parent->size().y() - dh->getTarget()->size().y()) {
                y = target_parent->size().y() - dh->getTarget()->size().y();
            }
        ...
        if (resized) {
            ...
            if (lp)
                ...
                dh->getTarget()->performLayout(EDITOR->gui()->nvgContext());
            }
        }
    }
}
