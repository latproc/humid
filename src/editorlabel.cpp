...
EditorLabel::EditorLabel(NamedObject *owner, Widget *parent, const std::string nam,
    ...
    : Label(parent, caption), EditorWidget(owner, "LABEL", nam, this, lp),
      mBackgroundColor(nanogui::Color(0, 0)), mTextColor(nanogui::Color(0, 0)), alignment(1),
      valign(1), wrap_text(true) {}

bool EditorLabel::mouseButtonEvent(const nanogui::Vector2i &p, int button, bool down,
                                   int modifiers) {
...
bool EditorLabel::mouseMotionEvent(const nanogui::Vector2i &p, const nanogui::Vector2i &rel,
...
void EditorLabel::loadProperties(PropertyFormHelper *properties) {
    ...
    if (w) {
        properties->addVariable<std::string>(
            "Caption", [&](std::string value) mutable { setCaption(value); },
