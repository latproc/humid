...
EditorFrame::EditorFrame(NamedObject *owner, nanogui::Widget *parent, const std::string nam,
                         LinkableProperty *lp)
    : nanogui::Widget(parent), EditorWidget(owner, "FRAME", nam, this, lp) {}

bool EditorFrame::mouseButtonEvent(const nanogui::Vector2i &p, int button, bool down,
                                   int modifiers) {
...
bool EditorFrame::mouseMotionEvent(const nanogui::Vector2i &p, const nanogui::Vector2i &rel,
...
void EditorFrame::loadProperties(PropertyFormHelper *properties) {
    ...
    if (w) {
        properties->addVariable<float>(
            "Value", [&](float value) mutable {}, [&]() -> float { return 0.0f; });
