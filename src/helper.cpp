...
Structure *firstScreen() {
    for (auto item : hm_structures) {
        Structure *s = item;
        assert(s);
        if (s->getStructureDefinition() && (s->getStructureDefinition()->getName() == "SCREEN" ||
                                            s->getStructureDefinition()->getBase() == "SCREEN"))
            return s;
    }
    return 0;
}
...
Structure *findStructureFromClass(std::string class_name) {
    for (auto item : hm_structures) {
        Structure *s = item;
        assert(s);
        if (s->getStructureDefinition() && (s->getStructureDefinition()->getName() == class_name ||
...
Structure *findStructure(const std::string &seek) {
    ...
    if (!found)
        found = findStructure(seek, builtin_structures);
    return found;
}
