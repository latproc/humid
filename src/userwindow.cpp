...
bool UserWindowWin::keyboardEvent(int key, int scancode, int action, int modifiers) {
    if (action == GLFW_PRESS || action == GLFW_REPEAT) {
        if (EDITOR && EDITOR->isEditMode()) {
            ...
            if (action == GLFW_PRESS && (key == GLFW_KEY_BACKSPACE || key == GLFW_KEY_DELETE)) {
                ...
                while (--i != start) {
                    ...
                    if (pos > 0) {
                        if (backward) {
                            if (pos > minpos && (start_pos == -1 || pos <= start_pos)) {
                                minpos = pos;
                                minidx = i;
                            }
                            if ((pos > maxpos || maxpos == -1) && pos >= start_pos) {
                                maxpos = pos;
                                maxidx = i;
                            }
                        }
                ...
                if (backward) {
                    ...
                    if (maxidx != start)
                        new_sel = maxidx;
                    else if (minidx != start)
                        new_sel = minidx;
                }
                if (new_sel >= 0 && new_sel != start) {
                    EditorWidget *ew = 0;
                    if (start >= 0)
                        ew = dynamic_cast<EditorWidget *>(childAt(start));
                    if (ew)
                        ew->deselect();
            ...
            else if (uw->hasSelections() && (key == GLFW_KEY_LEFT || key == GLFW_KEY_UP ||
                                             key == GLFW_KEY_RIGHT || key == GLFW_KEY_DOWN)) {
                ...
                for (auto item : uw->getSelected()) {
                    ...
                    case GLFW_KEY_LEFT:
                        if (w->position().x() > 0)
                            w->setPosition(
                                nanogui::Vector2i(w->position().x() - 1, w->position().y()));
                        break;
                    case GLFW_KEY_RIGHT:
                    ...
                    case GLFW_KEY_UP:
                        if (w->position().y() > w->parent()->theme()->mWindowHeaderHeight)
                            w->setPosition(
                                nanogui::Vector2i(w->position().x(), w->position().y() - 1));
                        break;
                    case GLFW_KEY_DOWN:
        ...
        for (auto item : children()) {
            ...
            if (lp && lp->focused()) {
                lp->handleKey(key, scancode, action, modifiers);
                handled = true;
            }
        }
        if (!handled)
            return Window::keyboardEvent(key, scancode, action, modifiers);
    }
    return true;
}
...
bool UserWindowWin::mouseEnterEvent(const Vector2i &p, bool enter) {
    ...
    if (!focused() && enter && EDITOR->isEditMode() &&
        ((sw && sw->hasSelections()) || (ow && ow->hasSelections())))
        requestFocus();
...
UserWindow::UserWindow(EditorGUI *screen, nanogui::Theme *theme, UserWindowWin *uww)
    : Skeleton(screen, uww), LinkableObject(), gui(screen), current_layer(0),
      mDefaultSize(1024, 768), current_structure(0) {
    using namespace nanogui;
...
void UserWindow::update(const Value &value) {
    if (!EDITOR->isEditMode()) {
        ...
        if (s)
            EditorGUI::systemSettings()->getProperties().add("active_screen",
                                                             Value(s->getName(), Value::t_string));
    }
}
    ...
    while (n--) {
        ...
        if (ew->getDefinition()) {
            ...
            if (r != SymbolTable::Null && r.asString() == lp->tagName()) {
                if (ew->getRemote())
                    ew->getRemote()->unlink(ew->getRemote());
                ew->setRemote(lp);
...
CircularBuffer *UserWindow::addDataBuffer(const std::string name, CircularBuffer::DataType dt,
                                          size_t len) {
    auto found = data.find(name);
    if (found != data.end()) {
        CircularBuffer *buf = (*found).second;
        if (buf->getDataType() == dt)
            return buf;
...
void UserWindow::loadProperties(PropertyFormHelper *properties) {
    ...
    }
    {
        ...
        properties->addVariable<std::string>(
            label,
            [uw](std::string value) {
                std::cout << "setting file name for " << uw->structure()->getName() << " to "
                          << value << "\n";
                if (uw->structure())
                    uw->structure()->getInternalProperties().add("file_name",
                                                                 Value(value, Value::t_string));
            },
            [uw]() -> std::string {
                if (!uw->structure())
                    return "";
        ...
        properties->addVariable<std::string>(
            ...
            [uw](std::string value) {
                if (uw->structure() && uw->structure()->getStructureDefinition())
                    uw->structure()->getStructureDefinition()->getInternalProperties().add(
                        "file_name", Value(value, Value::t_string));
            },
            [uw]() -> std::string {
                if (!uw->structure() || uw->structure()->getStructureDefinition())
                    return "";
                const Value &vx =
                    uw->structure()->getStructureDefinition()->getInternalProperties().find(
                        "file_name");
                if (vx != SymbolTable::Null)
                    return vx.asString();
                return "";
    ...
    }
    {
        ...
        properties->addVariable<int>(
            ...
            [uw](int value) mutable {
                ...
                Structure *s = uw->structure();
                if (s)
                    s->getProperties().add("screen_id", value);
            },
            [uw]() -> int {
                Structure *s = uw->structure();
                if (s) {
...
Value UserWindow::getPropertyValue(const std::string &prop) {
    if (prop == "Screen Width")
        return getWindow()->width();
    if (prop == "Screen Height")
        return getWindow()->height();
    if (prop == "Window Width")
        return getWindow()->width();
    ...
    if (prop == "Channel")
        return current_screen->getProperties().lookup("channel");
