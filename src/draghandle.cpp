...
DragHandle::DragHandle(Widget *parent, PropertyMonitor *pm)
    : ToolButton(parent, ENTYPO_ICON_PLUS), mValue(0.0f),
      mHighlightedRange(std::make_pair(0.f, 0.f)), property_monitor(pm) {
...
Vector2i DragHandle::preferredSize(NVGcontext *) const { return Vector2i(48, 48); }

bool DragHandle::mouseDragEvent(const Vector2i &p, const Vector2i & /* rel */, int /* button */,
                                int /* modifiers */) {
    if (!mEnabled) {
        return false;
    }
    ...
    if (x > parent()->size().x())
        x = parent()->size().x();
    int y = p.y();
    if (y < parent()->theme()->mWindowHeaderHeight + 1)
        y = parent()->theme()->mWindowHeaderHeight + 1;
    if (y > parent()->size().y())
        y = parent()->size().y();
...
bool DragHandle::mouseButtonEvent(const Vector2i &p, int /* button */, bool down,
                                  int /* modifiers */) {
    if (!mEnabled) {
...
void DragHandle::draw(NVGcontext *ctx) {
    ...
    NVGpaint bg = nvgBoxGradient(ctx, mPos.x(), center.y() - 3 + 1, mSize.x(), 6, 3, 3,
...
*/
    ...
    NVGpaint knobShadow = nvgRadialGradient(ctx, knobPos.x(), knobPos.y(), kr - 3, kr + 3,
    ...
    NVGpaint knobReverse =
        nvgLinearGradient(ctx, mPos.x(), center.y() - kr, mPos.x(), center.y() + kr,
