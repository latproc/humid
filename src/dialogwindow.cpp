    ...
    if (sc && (s->getKind() == "SCREEN" || sc->getBase() == "SCREEN")) {
        ...
        for (auto param : sc->getLocals()) {
            ...
            if (kind == "FRAME") {
                ...
                if (w.asInteger(x) && h.asInteger(y)) {
                    ...
                    setSize(frame_size);
                    auto pos = nanogui::Vector2i((gui->width() - frame_size.x()) / 2,
                                                 (gui->height() - frame_size.y()) / 2);
                    setPosition(pos);
                }
                break;
            }
        ...
        for (auto param : sc->getLocals()) {
            ...
            if (!element_class) {
                ...
                if (!element_class) {
                    std::cout << "cannot find structure definition for " << kind
                              << " when loading screen " << s->getName() << "\n";
                }
            }
