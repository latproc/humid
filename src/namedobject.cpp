...
NamedObject::NamedObject(NamedObject *owner, const char *msg)
    : _named(false), parent(owner), name(msg) {
    if (!parent)
...
NamedObject::NamedObject(NamedObject *owner, const std::string &msg)
    : _named(false), parent(owner), name(msg) {
    if (!parent)
        addNamedObjectToMap(this, global_objects);
    else
...
void NamedObject::remove(const std::string &object_name) { child_objects.erase(object_name); }
...
NamedObject::NamedObject(const NamedObject &orig) { name = orig.name; }
/*
...
std::string NamedObject::nextName(NamedObject *o, const std::string prefix) {
    ...
    while (item != dict.end()) {
        if ((*item).second == o)
            return trial_name; // this name is already assigned to the object
...
std::map<std::string, NamedObject *> &NamedObject::locals() { return child_objects; }

bool NamedObject::changeName(NamedObject *o, const std::string &oldname,
