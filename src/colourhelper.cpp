...
ColourResult colourFromString(const std::string &colour) {
    ...
    if (colour[0] == '#') {
        if (len == 4 || len == 5) {
            ...
            c = {.r = parse(p), .g = parse(p), .b = parse(p), .a = len == 9 ? parse(p) : 255};
        }
        else {
        ...
        if (len == 3) {
            ...
            auto grey = dbl(val(*p++));
            c = {.r = grey, .g = grey, .b = grey, .a = dbl(val(*p))};
        }
        else if (len == 5) {
            auto grey = parse(p);
            c = {.r = grey, .g = grey, .b = grey, .a = parse(p)};
        }
        else {
            error += "Unrecognised colour " + colour;
        }
...
std::string stringFromColour(const nanogui::Color &colour) {
    ...
    std::stringstream ss;
    ss << '#' << std::hex << std::setfill('0') << std::setw(2) << static_cast<int>(colour.r() * 255)
       << std::setw(2) << static_cast<int>(colour.g() * 255) << std::setw(2)
...
ColourResult colourFromValue(const Value &colour) {
    ...
    if (colour != SymbolTable::Null) {
        ...
        if (tokens.size() == 4) {
            ...
            for (int i = 0; i < 4; ++i)
                fields[i] = std::atof(tokens[i].c_str());
            return std::make_pair(nanogui::Color(fields[0], fields[1], fields[2], fields[3]),
                                  error);
        }
