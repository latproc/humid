        ...
        if (uw && getScreen()) {
            ...
            if (EDITOR->isEditMode())
                EditorGUI::systemSettings()->getProperties().add(
                    "active_screen", Value(getScreen()->getName(), Value::t_string));
            uw->setStructure(getScreen());
            uw->refresh();
        }
    }

    void setScreen(Structure *s) { screen = s; }
...
ScreensWindow::ScreensWindow(EditorGUI *screen, nanogui::Theme *theme)
    : Skeleton(screen), Palette(PT_SINGLE_SELECT), gui(screen) {
...

    {
        ...
        palette_scroller->setFixedSize(
            Vector2i(window->width(),
                     window->height() - window->theme()->mWindowHeaderHeight - tool_button_size));
        palette_scroller->setPosition(
            Vector2i(5, window->theme()->mWindowHeaderHeight + 10 + tool_button_size));
...
UserWindow *ScreensWindow::getUserWindow() { return gui->getUserWindow(); }
...
void ScreensWindow::update() {
    ...
    for (auto item : hm_structures) {
        ...
        if (s->getKind() == "SCREEN" || (sc && sc->getBase() == "SCREEN")) {
            ...
            ScreenSelectButton *b =
                new ScreenSelectButton("BUTTON", this, cell, s->getName(), this);
            b->setEnabled(true);
    ...
    if (!palette_content->childCount()) {
        ...
        cell->setFixedSize(nanogui::Vector2i(button_width + 4, 35));
        ScreenSelectButton *b =
            new ScreenSelectButton("BUTTON", this, cell, screen->getName(), this);
        b->setEnabled(true);
        b->setFixedSize(nanogui::Vector2i(button_width, 30));
        b->setPosition(nanogui::Vector2i(2, 2));
        b->setPassThrough(true);
