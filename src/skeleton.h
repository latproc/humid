...
class SkeletonWindow : public nanogui::Window, public PanelScreen {
  public:
    ...
    void setMoveListener(std::function<void(nanogui::Window *)> f) { move_listener = f; }
...
class ClockworkClient : public nanogui::Screen {
  ...
  public:
    ...
    class Connection {
      public:
        Connection(ClockworkClient *, const std::string connection_name, const std::string ch,
                   std::string h, int p);
      ...
      protected:
        std::list<std::pair<std::string, std::function<void(std::string)>>>
    ...
    ClockworkClient(const nanogui::Vector2i &size, const std::string &caption,
                    bool resizeable = true, bool fullscreen = false);
    ...
    void queueMessage(const std::string &connection_name, const std::string s,
                      std::function<void(std::string)> f);

    void queueMessage(const std::string &connection_name, const char *s,
                      std::function<void(std::string)> f);

    std::string getIODSyncCommand(const std::string &connection_name, int group, int addr,
                                  bool which);
    std::string getIODSyncCommand(const std::string &connection_name, int group, int addr,
                                  int new_value);
    std::string getIODSyncCommand(const std::string &connection_name, int group, int addr,
