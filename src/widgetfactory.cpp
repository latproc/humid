...
WidgetParams::WidgetParams(Structure *structure, Widget *w, Structure *elem, EditorGUI *editor_gui,
                           const nanogui::Vector2i &offset_)
    : s(structure), window(w), element(elem), gui(editor_gui),
      ...
      font_size_val(element->getValue("font_size")), remote_name(element->getValue("remote")),
      wrap_v(element->getValue("wrap")), ivis_v(element->getValue("inverted_visibility")),
      value_type_val(element->getValue("value_type")), tab_pos_val(element->getValue("tab_pos")),
      x_scale_val(element->getValue("x_scale")), theme_name(element->getValue("theme")),
...
template <typename T> void prepare_remote_links(const WidgetParams &params, T *w) {
    auto remote_links = LinkManager::instance().remote_links(
        params.s->getStructureDefinition()->getName(), w->getName());
    if (remote_links) {
        ...
        for (auto &link_info : *remote_links) {
            ...
            if (linkable_property) {
                ...
                if (found_property != property_id_to_name.end()) {
                    linkable_property->link(new LinkableObject(new PropertyLinkTarget(
                        w, (*found_property).second, defaultForProperty(link_info.property_name))));
                }
            }
            else {
...
void createLabel(WidgetParams &params) {
    ...
    if (params.value_type != -1)
        el->setValueType(params.value_type);
    if (params.value_scale != 1.0)
        el->setValueScale(params.value_scale);
    if (params.tab_pos)
        el->setTabPosition(params.tab_pos);
    if (params.lp)
        params.lp->link(new LinkableText(el));
    if (params.border != SymbolTable::Null)
        el->setBorder(params.border.iValue);
    el->setInvertedVisibility(params.ivis);
    if (params.visibility)
        el->setVisibilityLink(params.visibility);
...
void createList(WidgetParams &params) {
    EditorList *el =
        new EditorList(params.s, params.window, params.element->getName(), params.lp, "");
    el->setName(params.element->getName());
    el->setDefinition(params.element);
    if (params.theme.get()) {
        el->setTheme(params.theme);
...
void createImage(WidgetParams &params) {
    EditorImageView *el =
        new EditorImageView(params.s, params.window, params.element->getName(), params.lp, 0);
    el->setName(params.element->getName());
    el->setDefinition(params.element);
    if (params.connection != SymbolTable::Null) {
        el->setConnection(params.connection.asString());
...
void createProgress(WidgetParams &params) {
    EditorProgressBar *ep =
        new EditorProgressBar(params.s, params.window, params.element->getName(), params.lp);
    ep->setDefinition(params.element);
    if (params.theme.get()) {
        ep->setTheme(params.theme);
    }
    ...
    if (params.format_val != SymbolTable::Null)
        ep->setValueFormat(params.format_val.asString());
    if (params.value_type != -1)
        ep->setValueType(params.value_type);
    if (params.value_scale != 1.0)
        ep->setValueScale(params.value_scale);
    if (params.tab_pos)
        ep->setTabPosition(params.tab_pos);
    if (params.border != SymbolTable::Null)
        ep->setBorder(params.border.iValue);
    ep->setInvertedVisibility(params.ivis);
    ep->setChanged(false);
    if (params.lp)
        params.lp->link(new LinkableNumber(ep));
    if (params.visibility)
        ep->setVisibilityLink(params.visibility);
    {
        Value bg_colour(params.element->getValue("bg_color"));
        if (bg_colour != SymbolTable::Null)
...
void createText(WidgetParams &params) {
    EditorTextBox *textBox =
        new EditorTextBox(params.s, params.window, params.element->getName(), params.lp);
    ...
    if (params.format_val != SymbolTable::Null)
        textBox->setValueFormat(params.format_val.asString());
    if (params.value_type != -1)
        textBox->setValueType(params.value_type);
    if (params.value_scale != 1.0)
        textBox->setValueScale(params.value_scale);
    setElementPosition(params, textBox, params.element);
    fixElementSize(textBox, params.element);
    if (params.font_size)
        textBox->setFontSize(params.font_size);
    if (params.tab_pos)
        textBox->setTabPosition(params.tab_pos);
    if (params.border != SymbolTable::Null)
        textBox->setBorder(params.border.iValue);
    ...
    if (params.visibility)
        textBox->setVisibilityLink(params.visibility);
    ...
    textBox->setCallback([textBox, params](const std::string &value) -> bool {
        ...
        char *rest = 0;
        {
            ...
            if (*rest == 0) {
                params.gui->queueMessage(
                    conn,
                    params.gui->getIODSyncCommand(conn, textBox->getRemote()->address_group(),
                                                  textBox->getRemote()->address(),
                                                  (int)(val * textBox->valueScale())),
        ...
        }
        {
            ...
            extern int debug;
            if (debug) {
                std::cout << "sending: " << value << " of size " << size << "\n";
            }
        }
        return false;
    });
...
void createPlot(WidgetParams &params) {
    EditorLinePlot *lp =
        new EditorLinePlot(params.s, params.window, params.element->getName(), nullptr);
    lp->setDefinition(params.element);
    if (params.theme.get()) {
        lp->setTheme(params.theme);
    }
    ...
    if (params.format_val != SymbolTable::Null)
        lp->setValueFormat(params.format_val.asString());
    if (params.value_type != -1)
        lp->setValueType(params.value_type);
    if (params.value_scale != 1.0)
        lp->setValueScale(params.value_scale);
    if (params.font_size)
        lp->setFontSize(params.font_size);
    if (params.tab_pos)
        lp->setTabPosition(params.tab_pos);
    if (params.x_scale)
        lp->setTimeScale(params.x_scale);
    {
    ...
    lp->setChanged(false);
    if (params.visibility)
        lp->setVisibilityLink(params.visibility);
...
void createButton(WidgetParams &params) {
    ...
    EditorButton *b =
        new EditorButton(params.s, params.window, params.element->getName(), params.lp,
                         (caption_v != SymbolTable::Null) ? caption_v.asString() : "");
    if (params.kind == "INDICATOR")
        b->setEnabled(false);
    else
        b->setEnabled(true);
    b->setDefinition(params.element);
    if (params.theme.get()) {
        b->setTheme(params.theme);
    }
    ...
    if (params.font_size)
        b->setFontSize(params.font_size);
    if (params.tab_pos)
        b->setTabPosition(params.tab_pos);
    if (params.border != SymbolTable::Null)
    ...
    if (params.format_val != SymbolTable::Null)
        b->setValueFormat(params.format_val.asString());
    if (params.value_type != -1)
        b->setValueType(params.value_type);
    if (params.value_scale != 1.0)
        b->setValueScale(params.value_scale);
    {
        const Value caption_v = params.element->getValue("caption");
        if (caption_v != SymbolTable::Null)
            b->setCaption(caption_v.asString());
    }
    {
    ...
    }
    {
        ...
        if (cmd != SymbolTable::Null && cmd.asString().length())
            b->setCommand(cmd.asString());
    }
    b->setupButtonCallbacks(params.lp, params.gui);
    b->setImageName(params.element->getValue("image").asString());
    if (params.visibility)
        b->setVisibilityLink(params.visibility);
...
void createComboBox(WidgetParams &params) {
    EditorComboBox *el =
        new EditorComboBox(params.s, params.window, params.element->getName(), params.lp);
    el->setName(params.element->getName());
    el->setDefinition(params.element);
    if (params.theme.get()) {
        el->setTheme(params.theme);
