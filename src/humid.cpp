...
/* This file is formatted with:
    astyle --style=kr --indent=tab=2 --one-line=keep-blocks
...
class ItemProxy {
  public:
    ItemProxy(PropertyFormHelper *pfh, nanogui::Widget *w) : helper(pfh), item(w) { getAll(); }
    void getAll() {
        if (!item)
            return;
        x = item->position().x();
    ...
    std::list<path> files;
    {
        ...
        while (fd_iter != files_and_directories.end()) {
            ...
            if (is_regular_file(fp)) {
                ...
                if (!base_checked) {
                    base_checked = true;
                    settings->getProperties().add("project_base",
                                                  Value(fp.string().c_str(), Value::t_string));
...
bool applyWindowSettings(Structure *item, nanogui::Widget *widget) {
    if (widget) {
        ...
        if (vis_prop != SymbolTable::Null && vis_prop.asInteger(vis)) {
            if (!vis)
                std::cout << item->getName() << " is invisible\n";
...
int main(int argc, const char **argv) {
    ...
    // std:cout << "running from the " <<
    // boost::filesystem::current_path().string() << " directory\n";
    // std::string home_path(getenv("HOME"));
    // assert(boost::filesystem::current_path().string().find(home_path) ==
    // 0);
    ...
    generic.add_options()("help", "produce help message")(
        ...
        "remote host (localhost)")("cwport", po::value<int>(&cw_port)->default_value(5555),
                                   "clockwork port (5555)")(
    ...
    hidden.add_options()("source-file", po::value<std::vector<std::string>>(), "source file");
    ...
    if (vm.count("tags"))
        tag_file_name = vm["tags"].as<std::string>();
    if (vm.count("run_only"))
        run_only = vm["run_only"].as<int>();
    if (DEBUG_BASIC)
        std::cout << "Debugging\n";
...

        {
            if (vm.count("source-file")) {
                const std::vector<std::string> &files(
                    vm["source-file"].as<std::vector<std::string>>());
                for (auto s : files) {
                    int found = s.rfind("/");
                    if (found == s.length() - 1)
                        s.erase(found);
            ...
            // if necessary create a project settings structure to
            // store the nominated connection details
            ...
            if (!project_settings) {
                ...
                if (cw_port && hostname.length()) { // user supplied a host,
                                                    // setup the required
                                                    // connection object
            ...
            {
                const Value width_v =
                    EditorGUI::systemSettings()->getProperties().find("main_window_width");
                const Value height_v =
                    EditorGUI::systemSettings()->getProperties().find("main_window_height");
                width_v.asInteger(width);
                height_v.asInteger(height);
            }
            if (run_only) {
                const Value width_v =
                    EditorGUI::systemSettings()->getProperties().find("panel_width");
                const Value height_v =
                    EditorGUI::systemSettings()->getProperties().find("panel_height");
                width_v.asInteger(width);
                height_v.asInteger(height);
            }
            ...
            for (auto settings : Structure::findStructureClasses("THEME")) {
                ThemeManager::instance().addTheme(settings->getName(),
                                                  ThemeManager::instance().createTheme(settings));
            }
            ...
            if (remote_screen == SymbolTable::Null) {
                EditorGUI::systemSettings()->getProperties().add(
            ...
            if (!app->getUserWindow()->structure()) {
                const Value &active(
                    EditorGUI::systemSettings()->getProperties().find("active_screen"));
