...
EditorProgressBar::EditorProgressBar(NamedObject *owner, Widget *parent, const std::string nam,
                                     LinkableProperty *lp)
    : ProgressBar(parent), EditorWidget(owner, "PROGRESS", nam, this, lp),
      fg_color(nanogui::Color(192, 192, 192, 255)), bg_color(nanogui::Color(240, 240, 240, 255)) {}

bool EditorProgressBar::mouseButtonEvent(const nanogui::Vector2i &p, int button, bool down,
                                         int modifiers) {
...
bool EditorProgressBar::mouseMotionEvent(const nanogui::Vector2i &p, const nanogui::Vector2i &rel,
...
void EditorProgressBar::draw(NVGcontext *ctx) {
    ...
    int a = border / 2 + 1;
    NVGpaint paint = nvgBoxGradient(ctx, mPos.x() + 1, mPos.y() + 1, mSize.x() - 2, mSize.y(), 3, 4,
                                    bg_color, bg_color);
    nvgBeginPath(ctx);
    ...
    paint = nvgBoxGradient(ctx, mPos.x(), mPos.y(), barPos + 1.5f, mSize.y() - 1, 3, 4, fg_color,
                           fg_color);

    nvgBeginPath(ctx);
    nvgRoundedRect(ctx, mPos.x() + a, mPos.y() + a, barPos - 2 * a, mSize.y() - 2 * a,
                   mTheme->mButtonCornerRadius);
    nvgFillPaint(ctx, paint);
    nvgFill(ctx);
...
void EditorProgressBar::loadProperties(PropertyFormHelper *properties) {
    ...
    if (w) {
        ...
        properties->addVariable<float>(
            "Value", [&](float value) mutable { setValue(value); },
            [&]() -> float { return value(); });
        properties->addVariable<nanogui::Color>(
            "Foreground colour", [&](const nanogui::Color &value) mutable { setColor(value); },
