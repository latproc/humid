  ...
  public:
    ...
    void draw(NVGcontext *ctx) {
        ...
        nvgRoundedRect(ctx, current->position().x() + 1, current->position().y() + 1,
                       current->size().x() + 1, current->size().y() + 1,
                       current->theme()->mButtonCornerRadius);
...
class Editor {
  ...
  public:
    ...
    void setEditMode(bool which) {
        mEditMode = which;
        if (!which && widget_selector)
            widget_selector->select(0);
    }

    bool isEditMode() { return mEditMode; }

    bool isCreateMode() { return mCreateMode; }

    void setCreateMode(bool which) { mCreateMode = which; }
...
class EditorWidget {
  ...
  public:
    ...
    virtual bool editorMouseMotionEvent(nanogui::Widget *widget, const nanogui::Vector2i &p,
...
class EditorButton : public nanogui::Button, public EditorWidget {
  ...
  public:
    EditorButton(Widget *parent, const std::string &caption = "Untitled", int modbus_address = 0,
                 int icon = 0)
        : Button(parent, caption, icon), dh(0), handles(9), handle_coordinates(9, 2),
          addr(modbus_address) {}
    ...
    virtual bool mouseButtonEvent(const nanogui::Vector2i &p, int button, bool down,
        ...
        else if (down && EDITOR->selector())
            EDITOR->selector()->select(this);
...
template <typename T> class Selectable : public T {};
template <> class Selectable<EditorButton> : public EditorButton {
  public:
    Selectable(nanogui::Widget *parent, const std::string &caption = "Untitled",
               int modbus_address = 0, int icon = 0)
        : EditorButton(parent, caption, modbus_address, icon), mSelected(false) {}
    virtual bool mouseButtonEvent(const nanogui::Vector2i &p, int button, bool down,
                                  int modifiers) override {
        ...
        if (editorMouseButtonEvent(this, p, button, down, modifiers))
            return nanogui::Button::mouseButtonEvent(p, button, down, modifiers);
        else if (down && EDITOR->selector()) //EDITOR->selector()->select(this);
            mSelected = !mSelected;
        return true;
    }
...
class ClockworkExample : public nanogui::Screen {
  ...
  public:
    ClockworkExample()
        : nanogui::Screen(nanogui::Vector2i(1024, 768), "NanoGUI Test", kRESIZEABLE, kWINDOWED),
        ...
        tb->setChangeCallback([](bool state) { EDITOR->setEditMode(state); }
        ...
        tb->setFixedSize(Vector2i(32, 32));
        tb->setChangeCallback([](bool state) { EDITOR->setCreateMode(state); });
        ...
        properties->addVariable("Focussed Btn Gradient Top Colour",
                                window->theme()->mButtonGradientTopFocused);
        properties->addVariable("Focussed Btn Bottom Colour",
                                window->theme()->mButtonGradientBotFocused);
        properties->addVariable("Btn Gradient Top Colour",
                                window->theme()->mButtonGradientTopUnfocused);
        properties->addVariable("Btn Gradient Bottom Colour",
                                window->theme()->mButtonGradientBotUnfocused);
        properties->addVariable("Pushed Btn Top Colour", window->theme()->mButtonGradientTopPushed);
        properties->addVariable("Pushed Btn Bottom Colour",
                                window->theme()->mButtonGradientBotPushed);
    ...
    nanogui::Window *createPanelPage(const char *name, const char *filename = 0,
    ...
    bool importModbusInterface(std::istream &init, Widget *palette_content, Widget *container) {
        ...
        while (init.getline(buf, 200, '\n')) {
            ...
            {
                Selectable<EditorButton> *b =
                    new Selectable<EditorButton>(palette_content, tag_name, 0);
                b->setEnabled(false);
            ...
            case '0': {
                ...
                if (p != std::string::npos) {
                    ...
                    EditorButton *b =
                        new EditorButton(container, tag_name.substr(p + 5), address_int);
    ...
    nanogui::ref<nanogui::Window> getProperties() { return property_window; }

    ~ClockworkExample() { mShader.free(); }
    ...
    virtual void drawContents() override {
        {
            ...
            mvp.topLeftCorner<3, 3>() =
                Matrix3f(nanogui::AngleAxisf((float)glfwGetTime(), Vector3f::UnitZ())) * 0.25f;
            ...
            if (program_state == s_initialising) {
                ...
                subscription_manager =
                    new SubscriptionManager("PANEL_CHANNEL", eCLOCKWORK, "localhost", 5555);
                ...
                subscription_manager->monit_setup->addResponder(ZMQ_EVENT_DISCONNECTED,
                                                                disconnect_responder);
                subscription_manager->monit_setup->addResponder(ZMQ_EVENT_CONNECTED,
                                                                connect_responder);
                ...
                try {
                    {
                        ...
                        try {
                            ...
                            subscription_manager->setup().getsockopt(ZMQ_TYPE, &sock_type,
                                                                     &param_size);
                            ++item;
                            subscription_manager->subscriber().getsockopt(ZMQ_TYPE, &sock_type,
                                                                          &param_size);
                            ++item;
                            ...
                            std::cerr << "zmq exception " << zmq_errno() << " "
                                      << zmq_strerror(zmq_errno()) << " for item " << item
                                      << " in SubscriptionManager::checkConnections\n";
                        }
                    }
                            ...
                            if (MessageEncoding::getCommand(data, op, &message)) {
                                if (op == "STATE" && message->size() == 2) {
                                    ...
                                    if (val.kind == Value::t_string)
                                        output << "\""
                                               << escapeNonprintables(val.asString().c_str())
                                               << "\"";
                                    else
    ...
    void queueMessage(std::string s) { messages.push_back(s); }

    void queueMessage(const char *s) { messages.push_back(s); }
