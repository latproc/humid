...
nanogui::Vector2i fixPositionInWindow(const nanogui::Vector2i &pos, const nanogui::Vector2i &siz,
                                      const nanogui::Vector2i &area) {
    nanogui::Vector2i res(pos);
    if (pos.x() < 0)
        res.x() = 0;
    if (pos.y() < 0)
        res.y() = 0;
    if (pos.x() + siz.x() > area.x())
        res.x() = area.x() - siz.x();
    if (pos.y() + siz.y() > area.y())
        res.y() = area.y() - siz.y();
    return res;
}

bool SkeletonWindow::mouseButtonEvent(const nanogui::Vector2i &p, int button, bool down,
                                      int modifiers) {
    if (button == GLFW_MOUSE_BUTTON_RIGHT && !down) {
        if (shrunk) {
            ...
            saved_size = fixedSize();
            saved_pos = position();
            setFixedSize(nanogui::Vector2i(80, 20));
            if (shrunk_pos != nanogui::Vector2i(0, 0))
                setPosition(shrunk_pos);
            shrunk = true;
        }
        requestFocus();
...
ConfirmDialog::ConfirmDialog(nanogui::Screen *screen, std::string msg)
    : Skeleton(screen), message(msg) {
    using namespace nanogui;
    ...
    ok_button->setCallback([this] { this->window->setVisible(false); });
...
WindowStagger::WindowStagger(const nanogui::Screen *s)
    : screen_widget(s), next_pos(20, 40), stagger(20, 20) {}
...
ClockworkClient::ClockworkClient(const Vector2i &size, const std::string &caption, bool resizeable,
                                 bool fullscreen)
    : nanogui::Screen(size, caption, resizeable, fullscreen), window(0), window_stagger(this) {
        ...
        if (EDITOR->isEditMode()) {
            ...
            if (mDragActive && action == GLFW_RELEASE && dropWidget != mDragWidget) {
                if (mDragWidget && mDragWidget->parent()) {
                    mDragWidget->mouseButtonEvent(mMousePos -
                                                      mDragWidget->parent()->absolutePosition(),
...
bool ClockworkClient::mouseButtonEvent(const nanogui::Vector2i &p, int button, bool down,
                                       int modifiers) {
...
ClockworkClient::Connection::Connection(ClockworkClient *cc, const std::string connection_name,
                                        const std::string ch, std::string h, int p)
    : startup(sINIT), owner(cc), name(connection_name), channel_name(ch), host_name(h), port(p),
      sm(0), disconnect_responder(0), iosh_cmd(0), cmd_interface(0), g_iodcmd(0),
      command_state(WaitingCommand), last_update(0), first_message_time(0),
      message_time_scale(1000), local_commands("inproc://local_cmds"), needs_refresh(false) {
...
zmq::socket_t *ClockworkClient::Connection::commandInterface() { return cmd_interface; }
...
ClockworkClient::Connection *ClockworkClient::setupConnection(Structure *s_conn) {
    ...
    if (!(isNull(chn) || isNull(host))) {
        Connection *conn =
            new Connection(this, s_conn->getName(), chn.asString(), host.asString(), port);
        conn->setDefinition(s_conn);
        conn->SetupInterface();
        usleep(1000); //TBD is this necessary? do it properly...

        SubscriptionManager *sm = new SubscriptionManager(chn.asString().c_str(), eCLOCKWORK,
                                                          host.asString().c_str(), port);
...
bool ClockworkClient::Connection::update() {
    ...
    if (update_time - last_update > 10000) {
        if (update_time - last_update > 15000)
...
zmq::socket_t *ClockworkClient::Connection::getCommandSocket() const { return iosh_cmd; }
...
void ClockworkClient::idle() {
    ...
    if (program_state == s_initialising) {
        ...
        uint64_t now = microsecs();
        if (now - last_check_new_connections >
            1000000) { // attempt any new connections every second
            if (setupConnections(project_settings))
            ...
            const std::pair<std::string, Connection *> item = *iter++;
            {
...

                {
                    ...
                    try {
                        ...
                        else if (subscription_manager->setupStatus() ==
                                 SubscriptionManager::e_done) {
                            int loop_counter = 400;
                            if (conn->getStartupState() == sSTARTUP)
                                conn->refreshData();
                            conn->handleCommand(this);
                            while (loop_counter--) {
                                if (!conn->handleSubscriber())
                                    break;
                            }
                        }
                    }
                        ...
                        if (zmq_errno() != EINTR) {
                            std::cerr << "zmq exception " << zmq_errno() << " "
                                      << zmq_strerror(zmq_errno()) << " polling connection "
                                      << conn->getName() << "\n";
                        }
                    }
...
void ClockworkClient::Connection::queueMessage(const std::string s,
...
ClockworkClient::Connection *
findConnection(const std::string &connection_name,
               std::map<std::string, ClockworkClient::Connection *> &connections) {
...
void ClockworkClient::queueMessage(const std::string &connection_name, const std::string s,
                                   std::function<void(const std::string)> f) {
    Connection *conn = findConnection(connection_name, connections);
    if (conn)
        conn->queueMessage(s, f);
}

void ClockworkClient::queueMessage(const std::string &connection_name, const char *s,
                                   std::function<void(const std::string)> f) {
    Connection *conn = findConnection(connection_name, connections);
    if (conn)
        conn->queueMessage(s, f);
...
char *ClockworkClient::Connection::sendIODMessage(const std::string &s) {
    ...
    if (g_iodcmd)
        ...
        if (DEBUG_BASIC)
            std::cerr << "IOD interface not ready\n";

        return strdup("IOD interface not ready\n");
    }
}

char *ClockworkClient::sendIOD(const std::string &connection_name, int group, int addr,
                               int new_value) {
    Connection *conn = findConnection(connection_name, connections);
