...
int CircularBuffer::bufferIndexFor(int i) {
    ...
    if (i >= l)
        return back; /* looking past the end of the buffer */
...
void CircularBuffer::addSample(long time, double val) {
    ...
    if (zero_time == 0) {
        zero_time = time;
        start_time = microsecs() / 1000;
    }
    if (start_trigger && start_trigger->active()) {
        Value v(val);
        start_trigger->check(v);
        if (start_trigger->justTriggered())
            thaw();
    }
    if (back == -1 || val < smallest_value)
        smallest_value = val;
    if (back == -1 || val > largest_value)
        largest_value = val;
    ...
    if (start_trigger && start_trigger->justTriggered()) {
        std::multimap<SampleTrigger::Event, Handler *>::iterator iter =
            handlers.find(SampleTrigger::START);
        while (iter != handlers.end()) {
            const std::pair<SampleTrigger::Event, Handler *> &item = *iter++;
            if (item.first != SampleTrigger::START)
                break;
    ...
    if (stop_trigger && stop_trigger->justTriggered()) {
        std::multimap<SampleTrigger::Event, Handler *>::iterator iter =
            handlers.find(SampleTrigger::STOP);
        while (iter != handlers.end()) {
            const std::pair<SampleTrigger::Event, Handler *> &item = *iter++;
            if (item.first != SampleTrigger::STOP)
                break;
            item.second->operator()();
        }
    }
...
int CircularBuffer::findMovement(double amount, int max_len) {
    ...
    while (idx != back && n < max_len) {
        ...
        if (idx == -1)
            idx = bufsize - 1;
        ++n;
        if (fabs(current - values[idx]) >= amount)
            return n;
    }
    return n;
}
...
int CircularBuffer::size() { return bufsize; }
...
int main(int argc, const char *argv[]) {
    ...
    ++tests;
    if (length(mybuf) != test_buffer_size) {
        fail(tests);
    }
    ++tests;
