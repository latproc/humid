...
Value defaultForProperty(const std::string &property) {
    ...
    if (found != property_defaults.end()) {
        return found->second;
    }
    return defaultForType(typeForProperty(property));
}

std::string format_caption(const std::string &caption, const std::string format_string,
                           int value_type, float value_scale) {
