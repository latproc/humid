...
static void invert_property_map(const std::map<std::string, std::string> &normal,
...
static void prepare_class_properties(const std::string &class_name,
...
std::ostream &Structure::operator<<(std::ostream &out) const { return out << name << " " << kind; }

std::ostream &operator<<(std::ostream &out, const Structure &s) { return s.operator<<(out); }

void StructureClass::addProperty(const char *p) { property_names.insert(p); }

void StructureClass::addProperty(const std::string &p) { property_names.insert(p.c_str()); }
...
bool Structure::getBoolProperty(const std::string name, bool default_value) {
    ...
    if (val != SymbolTable::Null) {
        ...
        case Value::t_string:
        case Value::t_symbol:
            if (val.sValue == "true" || val.sValue == "TRUE")
                return true;
            else if (val.sValue == "false" || val.sValue == "FALSE")
                return false;
            else
                return default_value;
        default:
...
const Value &Structure::getDefault(const char *name) {
    const auto &result =
        class_definition ? class_definition->getDefaults().find(name) : SymbolTable::Null;
    return result;
    ;
}
...
bool writePropertyList(std::ostream &out, const SymbolTable &properties,
                       ...
                       const std::map<std::string, std::string> *link_map) {
        ...
        if (!isNull(item.second) && (link_map || item.second.asString() != "")) {
            if (link_map) {
                ...
                if (remote_name != link_map->end()) {
                    out << delim << item.first << ": $"
                        << Value((*remote_name).second, Value::t_symbol);
            ...
            if (item.first == "text" || item.first == "working_text")
                continue; // don't save text properties unless they were in a link map.
            if (item.second.kind == Value::t_string)
                out << delim << item.first << ": "
                    << item.second; // quotes are automatically added to string values
            else
                out << delim << item.first << ": " << item.second;
            delim = property_delim;
        }
    }
    if (delim == property_delim)
...
void addStructureClass(StructureClass *new_class, std::list<StructureClass *> &classes) {
    if (new_class->getBase().empty()) {
        ...
        while (iter != classes.end()) {
            ...
            else if (sc->getBase() == new_class->getBase() ||
                     sc->getBase() == new_class->getName()) {
                classes.insert(iter++, new_class);
                return;
            }
            else {
...
std::list<std::string> checkStructureClasses() {
    ...
    for (const auto sc : hm_classes) {
        if (!sc->getBase().empty()) {
            ...
            if (!found) {
                std::stringstream ss;
                ss << "Base structure: " << sc->getBase()
                   << " not found for class: " << sc->getName();
