...
LinkableProperty::LinkableProperty(const std::string group, int object_type,
    ...
    : EditorObject(nullptr, name), group_name(group), kind(object_type), tag_name(name),
      address_str(addr_str), data_type_name(dtype),
...
LinkableProperty::~LinkableProperty() {
    if (links.size()) {
        std::cout << "deleting linkable property " << name << " with " << links.size()
                  << " links\n";
    }
}
...
void LinkableProperty::setAddress(int a) {
    modbus_address = a;
    setAddressStr(address_group(), a);
}

int LinkableProperty::address_group() const {
    if (address_str.at(0) == '\'')
        return address_str.at(1) - '0';
    else
