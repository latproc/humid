...
int ModbusAddress::next_discrete =
    ModbusAddress::first_auto_address;                            // next free address in the group
int ModbusAddress::next_coil = ModbusAddress::first_auto_address; // next free address in the group
int ModbusAddress::next_input_register =
    ModbusAddress::first_auto_address; // next free address in the group
int ModbusAddress::next_holding_register =
    ModbusAddress::first_auto_address; // next free address in the group
...
ModbusAddress::Group groupFromInt(int g) {
    if (g == (int)ModbusAddress::coil) {
        return ModbusAddress::coil;
    }
    if (g == (int)ModbusAddress::discrete) {
        return ModbusAddress::discrete;
    }
    if (g == (int)ModbusAddress::input_register) {
        return ModbusAddress::input_register;
    }
    if (g == (int)ModbusAddress::holding_register) {
        return ModbusAddress::holding_register;
    }
...
int ModbusAddress::next(ModbusAddress::Group g) {
    switch (g) {
    case discrete:
        return next_discrete;
    case coil:
        return next_coil;
    case input_register:
        return next_input_register;
    case holding_register:
        return next_holding_register;
    default:;
    }
    return 0;
}

ModbusAddress ModbusAddress::alloc(ModbusAddress::Group g, unsigned int n, ModbusAddressable *ma,
    ...
    case coil:
        ...
        break;
    case input_register:
        address = next_input_register;
        next_input_register += n;
        break;
    case holding_register:
        address = next_holding_register;
        next_holding_register += n;
        break;
    case string:
        address = next_input_register;
        next_input_register += n;
    ...
    DBG_MODBUS << "Modbus allocated " << n << " addresses in group " << (int)g << " starting at "
               << address << "\n";
...
ModbusAddress::ModbusAddress(ModbusAddress::Group g, int a, int n, ModbusAddressable *o, Source src,
                             const std::string &nam, ModbusExport::Type exType, int i)
    : group(g), address(a), allocated(n), exportType(exType), offset(i), source(src), owner(o),
      name(nam) {
    switch (g) {
    case discrete:
        if (address >= next_discrete) {
            next_discrete = address + 1;
        }
        break;
    case coil:
    ...
    case input_register:
        if (address >= next_input_register) {
            next_input_register = address + n;
        }
        break;
    case holding_register:
        if (address >= next_holding_register) {
            next_holding_register = address + n;
        }
        break;
    case string:
        if (address >= next_holding_register) {
    ...
    DBG_MODBUS << "Modbus recording " << n << " addresses in group " << (int)g << " starting at "
               << address << "\n";
    int index = ((int)g << 16) + address;
    if (address < first_auto_address) {
...
ModbusAddress ModbusAddress::lookup(int group, int address) {
    ...
    DBG_MODBUS << "looking up modbus " << group << ":" << address
               << ". Auto size: " << automatic_mappings.size() << " local mappings "
               << user_mappings.size() << "\n";
...
std::ostream &ModbusAddress::operator<<(std::ostream &out) const {
    out << name << ": " << group << std::setfill('0') << std::setw(5) << address
        << " src:" << source;
    return out;
}
