    ...
    if (state_history_count > 0) {
        ...
        if (thrash.thrashing) {
            ...
            out << " (~" << thrash.transitions_per_sec << "/s)\n"
                << "  path: " << thrash.path << "\n"
                << "  tip: SHOW CYCLING; plant-wide fast thrash only\n";
        }
        uint64_t now = microsecs();
...
MachineInstance::analyseStateThrash(uint64_t short_hold_us, size_t min_short,
    ...
    const size_t almost_all = (state_history_count * 3 + 3) / 4;
    const bool almost_all_short =
        info.short_holds >= almost_all && info.short_holds >= min_short;
    const bool holds_uniformly_short =
        info.avg_hold_us < short_hold_us && info.max_hold_us < short_hold_us * 4;
    const bool ring_burned_quickly = info.span_us > 0 && info.span_us < 1000000ULL;
    const bool fast_enough = info.transitions_per_sec >= 10.0;
    const bool still_thrashing_now = info.current_dwell_us < 500000ULL;
