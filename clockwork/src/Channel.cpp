...
class chn_scoped_lock {
  public:
    chn_scoped_lock(const char *loc, boost::mutex &mut)
        : location(loc), mutex(mut), lock(mutex, boost::defer_lock) {
        lock.lock();
    }
    ~chn_scoped_lock() { lock.unlock(); }
    std::string location;
    boost::mutex &mutex;
...
bool RemoteClockworkCommandFilter::filter(char **buf, size_t &len) {
    char *data = *buf;
    {
        ...
        fl.f() << "Channel " << channel->getName() << " subscriber received: " << data
               << " len:" << len << "\n";
    }
    if (len == 0) {
        return true;
...
ChannelImplementation::ChannelImplementation()
    : monitors_exports(false), last_modified(0), last_checked(0), authority(0) {
    authority = random();
}
...
void Channel::syncInterfaceProperties(MachineInstance *m, std::list<std::string> &messages) {
    ...
    if (definition()->updates_names.count(m->getName()) ||
        ...
        if (!mc) {
            std::string msg = MessageLog::instance()->add("Warning: Interface ", interface_name,
                                                          " is not defined");
            DBG_CHANNELS << msg << "\n";
            return;
        }
...
void Channel::stop() {
    ...
    disableShadows();
    if (isClient()) {
        if (definition()->monitors() || monitors()) {
            stopSubscriber();
        }
    }
    else {
        if (definition()->updates() || updates() || definition()->shares_machines() ||
            shares_machines()) {
            stopSubscriber();
        }
    }
...
std::ostream &operator<<(std::ostream &out, const Channel &m) { return m.operator<<(out); }

bool Channel::operator==(const Channel &other) { return false; }
...
ChannelDefinition::ChannelDefinition(const char *n, ChannelDefinition *prnt)
    ...
    if (!all) {
        all = new std::map<std::string, ChannelDefinition *>;
    }
    (*all)[n] = this;
...
void ChannelDefinition::addFeature(Feature f) { features.insert(f); }

void ChannelDefinition::removeFeature(Feature f) { features.erase(f); }

bool ChannelDefinition::hasFeature(Feature f) const { return features.count(f) != 0; }
...
bool ChannelImplementation::monitors() const {
    return monitors_exports || !monitors_names.empty() || !monitors_patterns.empty() ||
           !monitors_properties.empty();
}

bool ChannelImplementation::updates() const { return !updates_names.empty(); }

bool ChannelImplementation::shares_machines() const { return !shares_names.empty(); }

class ChannelConnectMonitor : public EventResponder {
  public:
    ChannelConnectMonitor(Channel *channel) : chn(channel) {}
    void operator()(const zmq_event_t &event_, const char *addr_) { chn->addConnection(); }
    Channel *chn;
};
class ChannelDisconnectMonitor : public EventResponder {
  public:
    ChannelDisconnectMonitor(Channel *channel) : chn(channel) {}
    void operator()(const zmq_event_t &event_, const char *addr_) { chn->dropConnection(); }
    Channel *chn;
};
    ...
    */
    if (isClient()) {
        ...
        communications_manager =
            new SubscriptionManager(definition()->name.c_str(), eCHANNEL, "*", port);
    }
#if 1
    {
        int retry = 1;
...
void ChannelDefinition::instantiateInterfaces() {
    ...
    if (!all) {
        return; // no channel definitions to look through
    }
    std::map<std::string, ChannelDefinition *>::iterator iter = all->begin();
    while (iter != all->end()) {
        const std::pair<std::string, ChannelDefinition *> item = *iter++;
        std::map<std::string, Value>::iterator updated_machines =
            item.second->updates_names.begin();
            ...
            if (!found) { // instantiate a shadow to represent this machine
                if (item.second) {
                    ...
                    MachineInstance *m = MachineInstanceFactory::create(
                        instance_name.first.c_str(), instance_name.second.asString().c_str(),
...
Channel *Channel::create(unsigned int port, ChannelDefinition *defn) {
    ...
    if (defn->monitors_exports) {
        chn->monitors_exports = true;
    }
    chn->modified();
    chn->setupShadows();
    chn->setupFilters();
    ...
    if (communications_manager) {
        ...
        if (!definition()->isPublisher()) {
            ...
            if (m->isShadow() && m->ownerChannel() == this) {
                if (auth) {
                    return; // do not reflect authorised property changes back to the owner
                }
        ...
        if (isClient()) {
            if (communications_manager->setupStatus() == SubscriptionManager::e_done) {
                ...
                DBG_CHANNELS << "Not sending '" << cmd
                             << "' since the subscriber is not connected yet\n";
            }
        }
...
void Channel::sendPropertyChange(MachineInstance *machine, const Value &key, const Value &val,
    ...
    while (iter != all->end()) {
        Channel *chn = (*iter).second;
        iter++;
        if (!chn->definition()->hasFeature(ChannelDefinition::ReportPropertyChanges)) {
            continue;
        }
        if (chn->current_state != ChannelImplementation::ACTIVE) {
            continue;
        }
...
void Channel::sendPropertyChanges(MachineInstance *machine) {
    ...
    while (iter != all->end()) {
        ...
        bool do_properties =
            chn->definition()->hasFeature(ChannelDefinition::ReportPropertyChanges);
        bool do_local_properties =
            chn->definition()->hasFeature(ChannelDefinition::ReportLocalPropertyChanges);
        if (chn->current_state == ChannelImplementation::DISCONNECTED) {
            continue;
        }
        if (!do_modbus && !do_properties) {
            continue;
        }
        ...
        std::map<MachineInstance *, MachineRecord *>::iterator found =
            chn->throttled_items.find(machine);
...
bool Channel::doesUpdate() { return does_update; }
...
void Channel::sendModbusUpdate(MachineInstance *machine, const std::string &property_name,
    ...
    while (iter != all->end()) {
        ...
        if (!chn->definition()->hasFeature(ChannelDefinition::ReportModbusUpdates)) {
            continue;
        }
        ...
        if (chn->throttle_time && machine->needsThrottle()) {
            DBG_CHANNELS << chn->getName() << " throttling " << machine->getName() << " "
                         << property_name << "\n";
            if (!chn->throttled_items[machine]) {
                chn->throttled_items[machine] = new MachineRecord(machine);
            }
...
void Channel::sendStateChange(MachineInstance *machine, std::string new_state, uint64_t auth) {
    ...
    while (iter != all->end()) {
        ...
        if (!chn->definition()->hasFeature(ChannelDefinition::ReportStateChanges)) {
            continue;
        }
        ...
        if (chn->filtersAllow(machine)) {
            ...
            if (!chn->definition()->isPublisher()) {
                if (machine->isShadow()) {
                    if (machine->ownerChannel() == chn && auth) {
                        continue;
                    }
...
void Channel::sendCommand(MachineInstance *machine, std::string command, std::list<Value> *params,
    ...
    if (machine->isShadow()) {
        DBG_CHANNELS << " redirecting " << command << " to " << machine->getName()
                     << " owner channel\n";
        ...
        while (iter != all->end()) {
            ...
            if (command == "UPDATE" &&
                !chn->definition()->hasFeature(ChannelDefinition::ReportModbusUpdates)) {
                continue;
            }
...
void ChannelDefinition::setKey(const char *s) { psk = s; }

void ChannelDefinition::setIdent(const char *s) { identifier = s; }
void ChannelDefinition::setVersion(const char *s) { version = s; }
...
void Channel::setupShadows() {
    ...
    if (!definition_) {
        definition_ = ChannelDefinition::find(this->_type.c_str());
    }
    if (!definition_) {
        char buf[150];
...
CommandSocketInfo::CommandSocketInfo(Channel *chn) : sock(0), index(0) {
    ...
    try {
        ...
        char errmsg[150];
        snprintf(errmsg, 150, "Exception binding a command socket for %s: %s",
                 chn->getName().c_str(), buf);
        MessageLog::instance()->add(errmsg);
        NB_MSG << errmsg << "\n";
    }
    DBG_CHANNELS << "Bound command channel socket (processing side) of " << chn->getName() << " to "
                 << buf << " at index " << index << "\n";
}
...
void Channel::checkCommunications() {
    ...
    if (!communications_manager->ready()) {
        return;
    }
    ...
    if (communications_manager->monit_setup->disconnected() ||
        ...
        if (isClient()) {
            if (communications_manager->monit_setup->disconnected() &&
                ...
                if (communications_manager->setupStatus() ==
                        SubscriptionManager::e_waiting_connect &&
...
void Channel::setupFilters() {
    ...
    if (definition()->monitor_linked.size()) {
        ...
        while (iter != definition()->monitor_linked.end()) {
            ...
            while (m_iter != MachineInstance::end()) {
                MachineInstance *machine = *m_iter++;
                if (machine && machine->listens.count(master) &&
                    !this->channel_machines.count(machine)) {
                    machine->publish();
                    this->channel_machines.insert(machine);
                }
            }
    ...
    does_monitor =
        monitors_exports || definition()->monitors_exports ||
        !(definition()->monitors_names.empty() && definition()->monitors_patterns.empty() &&
          ...
          ignores_patterns.empty() && monitors_names.empty() && monitors_patterns.empty() &&
          monitors_properties.empty());
            ...
            while (machines != MachineInstance::end()) {
                ...
                if (execute_pattern(rexp, machine->getName().c_str()) == 0) {
                    if (chn->channel_machines.count(machine)) {
                        ...
                        DBG_CHANNELS << "ignore pattern " << pattern << " matches "
                                     << machine->fullName() << " but it is not monitored\n";
                    }
                }
                else {
