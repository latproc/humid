...
*/
char *substitute_pattern(rexp_info *info, std::vector<std::string> &variables, const char *string,
                         const char *subst) {
    int matched_ok;
...
int is_integer(const char *string) { return matches(string, "^[-]{0,1}[0-9]+$"); }

int is_symbol(const char *string) { return matches(string, "[A-Za-z][A-Za-z0-9_.]*"); }
...
int main(int argc, char *argv[]) {
    ...
    for (i = 1; i < argc; i++) {
        if (expecting_pattern) {
            ...
            else if (find_matches(info, symbols, text) == 0) {
                display_matches(info, symbols);
            }
