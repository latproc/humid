...
Predicate::Predicate(const Value &v)
    : left_p(0), op(opNone), right_p(0), entry(v), mi(0), dyn_value(0), cached_entry(0),
      last_calculation(0), priority(0), lookup_error(false), needs_reevaluation(true),
      last_evaluation_time(0) {
    if (entry.kind == Value::t_symbol && entry.sValue == "DEFAULT") {
...
PredicateSymbolDetails::PredicateSymbolDetails(std::string n, std::string t, std::string e)
    : name(n), type(t), export_name(e) {}
...
void Predicate::findSymbols(std::set<PredicateSymbolDetails> &symbols,
                            const Predicate *parent) const {
    if (left_p) {
        left_p->findSymbols(symbols, this);
    }
    if (right_p) {
        right_p->findSymbols(symbols, this);
    }
    if (entry.kind == Value::t_symbol) {
    ...
    if (left_p) {
        ...
        if (entry.kind == Value::t_symbol) {
            if (entry.sValue == "TIMER") {
                ...
                std::map<std::string, PredicateSymbolDetails>::iterator item =
                    symbols.find(entry.sValue);
                if (item != symbols.end()) {
