...
class ModbusAddress {
  public:
    ...
    ModbusAddress()
        : group(none), address(0), allocated(0), exportType(ModbusExport::none), offset(0),
          source(unknown) {}
    ModbusAddress(Group group, int address, int len, ModbusAddressable *owner, Source source,
                  const std::string &name, ModbusExport::Type exType, int offset = 0);
    ...
    static ModbusAddress alloc(Group g, unsigned int n, ModbusAddressable *owner, Source src,
                               const std::string &full_name, ModbusExport::Type exType);
    ...
    bool operator<(const ModbusAddress &other) const {
        ...
        if (group == other.group && address < other.address) {
            return true;
        }
        return false;
    }
